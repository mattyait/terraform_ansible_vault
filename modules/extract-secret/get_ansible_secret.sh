...
eval "$(jq -r '@sh "FILE=\(.file) VAR=\(.var) VAULT_PASSWORD_FILE=\(.vault_password_file)"')"

# You need to have your env var ANSIBLE_VAULT_PASSWORD used in VAULT_PASSWORD_FILE
ansible localhost --vault-password-file=${VAULT_PASSWORD_FILE} -m debug -a var=${VAR} -e @${FILE} 2> /dev/null | sed 's/localhost | .* => \(.*\)/\1/' | jq -r .${VAR}
